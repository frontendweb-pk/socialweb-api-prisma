<template>
  <div class="mb-3">
    ...
    <div :class="[
      ...
      <select :disabled="!options.length" class="outline-none w-full p-2 bg-transparent" v-bind="$attrs"
        v-on="validationListeners" :name="name" :value="value">
        <option v-if="placeholder" :value="null" disabled>
          {{ placeholder }}
        </option>
        <option v-for="option in options" :key="JSON.stringify(option)" :value="optionValue!(option)">
