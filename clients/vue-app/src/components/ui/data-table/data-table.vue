            ...
            <tr class="hover:bg-indigo-50/50 text-sm text-slate-900" v-for="row in data" :key="JSON.stringify(row)">
                <template v-for="column in columns" :key="column.field">
                    <td class="p-3" v-if="$slots[column.field]">
                        <slot :name="column.field" :row="row" :column="column.field">{{ row[column.field] }}</slot>
                    </td>
                    <td class="p-3" v-else-if="column.field === 'created_at'">{{ transformDate(row[column.field] as
                        string) }}</td>
                    <td class="p-3" v-else-if="column.field === 'updated_at'">{{ transformDate(row[column.field] as
                        string) }}</td>
