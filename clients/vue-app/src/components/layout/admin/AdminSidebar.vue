            ...
            <ul>
                <template v-for="menu in adminMenus" :key="menu.name">
                    ...
                    <ul class="mb-4 border-b border-b-gray-100 pb-4" v-if="menu.children">
                        <NavItem class="py-2 text-sm flex items-center gap-2" v-for="child in menu.children"
                            :key="child.name" :to="child.href" isMenu>
...
const classes = computed(() => {
    return {
        'fixed top-0 left-0 h-screen w-64 bg-gray-800 text-white transition-transform duration-300 ease-in-out': true,
